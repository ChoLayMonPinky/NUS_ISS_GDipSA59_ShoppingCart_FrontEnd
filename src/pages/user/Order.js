...
export default function OrderHistory() {
  ...
  const getStatusInfo = (status) => {
    const statusInfo = {
      Processing: { icon: <Package className="w-5 h-5" />, text: 'Processing' },
      Shipped: { icon: <Truck className="w-5 h-5" />, text: 'Shipped' },
      OutForDelivery: { icon: <Truck className="w-5 h-5" />, text: 'Out for Delivery' },
  ...
  const OrderStatus = ({ status }) => {
    const statuses = ['Processing', 'Shipped', 'OutForDelivery', 'Delivered'];
  ...
  // 更新 submitReview 函数，将评分、评论内容以及产品 ID 发送到后端
  const submitReview = async () => {
    const reviewPayload = {
      productId: reviewData.itemId,  // product ID from the state
      rating: reviewData.rating,     // user rating
      comment: reviewData.comment    // user comment
    };
                    ...
                    {order.items.map((item) => (
                      <div key={item.productId} className="flex items-center justify-between bg-white p-4 rounded-lg shadow-sm">
                        ...
                        <div className="flex items-center space-x-4">
                          ...
                          {['PENDING','Delivered', 'Shipped', 'OutForDelivery'].includes(order.status) && (
                            <button
